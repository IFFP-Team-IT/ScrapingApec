        ...
        for sec in secteursActivite:
            ...
            while True:
                ...
                for j in range(0,len(c_page_URLs)):
                    ...
                    idtext = str(c_page_URLs[j].split("/")[-1].split("?")[0])
                    print("idtext ------------>>>   " + idtext)
                    ...
                    #
                    ## Added by Idris to test download file
                    #print("_____________________________________________________________________")
                    #print("_____________________________________________________________________")
                    #resume_file = (scr_hlp.get_element_attr("//*[contains(@class,'cv-profil')]/a[contains(@href,'filename')]","href"))
                    #print("resume_file ------------>>>   " + str(resume_file))
                    #resfile_url = "https://www.apec.fr" + str(resume_file)
                    #print("resfile_url ------------>>>   " + str(resfile_url))
                    #dd_resume_file = wget.download(resfile_url, out="/home/iao/extapec/downloads")                    
                    #print("_____________________________________________________________________")
                    #print("_____________________________________________________________________")
                    #dd_resume_file
                    ###
                    ## Other Method Works but PDF not readable
                    resume_file = (scr_hlp.get_element_attr("//*[contains(@class,'cv-profil')]/a[contains(@href,'filename')]","href"))
                    resfile_url = "https://www.apec.fr" + str(resume_file)
                    print("_____________________________________________________________________")
                    print("_____________________________________________________________________")
                    print("resume_file ------------>>>   " + str(resume_file))
                    print("resfile_url ------------>>>   " + str(resfile_url))
                    print("destination ------------>>>   " + "/home/iao/extapec/downloads/" + idtext + ".pdf")
                    #wget.download(resfile_url, "/home/iao/extapec/downloads/" + idtext +".pdf")
                    #print("_____________________________________________________________________")
                    #print("_____________________________________________________________________")
                    ###
                    ## Other Method but PDF not readable
                    #resfilename = Path("/home/iao/extapec/downloads/" + idtext +".pdf")
                    #response = requests.get(resfile_url)
                    #resfilename.write_bytes(response.content)
                    #print("_____________________________________________________________________")
                    #print("_____________________________________________________________________")
                    ###
                    #
                    #urllib.request.urlretrieve(resfile_url, "/home/iao/extapec/downloads/" + idtext +".pdf") #Works but not readable
                    # Other Method with function in scr_hlp Work but not readable also !!
                    scr_hlp.pdf_resume_Download(resfile_url, idtext)
                    print("_____________________________________________________________________")
                    print("_____________________________________________________________________")
                    ## End Added by Idris
                    #
                    ...
                    for e in scr_hlp.d.find_elements_by_xpath("//section[contains(@class,'section-moments')]//ngu-tile"):
                        ...
                        # Added by idris to try handling error if empty
#                        row_nb_mom.append(scr_hlp.get_element_text(".//span[./parent::div[contains(@class,'lieux')] or contains(@class,'school-type')]", e)) #location
                        if len(scr_hlp.get_element_text(".//span[./parent::div[contains(@class,'lieux')] or contains(@class,'school-type')]", e)) > 1:
                            row_nb_mom.append(scr_hlp.get_element_text(".//span[./parent::div[contains(@class,'lieux')] or contains(@class,'school-type')]", e)) #location
                        else:
                            row_nb_mom.append('') #location
