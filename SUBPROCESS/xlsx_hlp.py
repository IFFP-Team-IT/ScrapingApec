...
class xlsx_hlp:
    ...
    current = ""
    ws_main = None
    headers_main = ['datasource', 'name_file', 'profile_url', '_id', 'date_maj', 'nb_consult', 'date_der_consult',
                    'photo_url',
    ...
    @staticmethod
    def create_wb(filtername=""):  # creer un fichier excel avec 5 feuilles

        # create folder if not present
        if not os.path.isdir(xlsx_hlp.folder_name):
            os.mkdir(xlsx_hlp.folder_name)

        xlsx_hlp.current = os.path.join(xlsx_hlp.folder_name,
                                        xlsx_hlp.filename + filtername + ".xlsx")  # créée un fichier dans le dossier folder_name(qu'il faut créé s'il n'existe pas

        # os.path.join permet de créé un chemin avec les slach("/")
        # scr_hlp.scr_hlp.file_extract = xlsx_hlp.current
        # print(scr_hlp.scr_hlp.file_extract)
        ...
        if os.path.isfile(xlsx_hlp.current):
            ...
            xlsx_hlp.wb.close()
            scr_hlp.scr_hlp.pause_if_EXTRADEBUG("New File created")
            xlsx_hlp.open_wb()

    @staticmethod
    def open_wb():  # ouvre le fichier excel
    ...
    def save_wb():
        xlsx_hlp.wb.close()
        scr_hlp.scr_hlp.pause_if_EXTRADEBUG("Excel file saved")
        xlsx_hlp.open_wb()
