...
## Added by Idris - Change files and folder names
xlsx_hlp.folder_name = "Apec_CV_DB"
xlsx_hlp.filename = "extapec_"+datetime.now().strftime("%d%m%y")
    ...
    for loc in lieux:
        for sec in secteursActivite:
            ...
            while True:
                ...
                scr_hlp.load_page(list_page_URL%page_num, do_handle_login = False, refresh_also = False, wait_ele_xpath="//a[@class='actualLink' and contains(@href,'/detailProfil/')]", ele_count=20)
                ...
                for j in range(0,len(c_page_URLs)):
                    ...
                    scr_hlp.load_page(c_page_URLs[j], do_handle_login = False, refresh_also = False, wait_ele_xpath="//*[contains(@id,'photo-profil')]/img", ele_count=1)
                    ...
                    ## Added by idris - Saving after each profile
                    xlsx_hlp.save_wb()#saves after each profile
